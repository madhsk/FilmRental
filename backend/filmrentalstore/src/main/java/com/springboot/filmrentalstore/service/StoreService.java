...
public class StoreService implements IStoreService {
	...
	public Store addStore(StoreDTO storeDTO) throws ResourceNotFoundException {
		Address address = addressRepo.findById(storeDTO.getAddress().getAddressId())
		        .orElseThrow(() -> new ResourceNotFoundException("Address not found with id: " + storeDTO.getAddress().getAddressId()));
	...
	public List<StoreDTO> getStoreIdsByCountry(String country) throws ResourceNotFoundException {
		List<StoreDTO> storeIdDTOs = storeRepo.findAll().stream()
				.filter(store -> store.getAddress().getCity().getCountry().getCountry().equalsIgnoreCase(country))
				.map(store -> new StoreDTO(store.getStoreId(),store.getAddress(),store.getManager(),store.getLastUpdate(),store.getCustomers(),store.getStaffList())) // Map only storeId to
																									// StoreIdDTO
	...
	public List<CustomerStoreDTO> getCustomersByStoreId(Long storeId) throws ResourceNotFoundException {
		...
		List<Customer> customers = customerRepo.findByStore_StoreId(store.getStoreId());
